...
func (r *KubevirtMachineReconciler) AllocateIPsForMachine(machine *infrav1.KubevirtMachine, config *infrav1.VirtualMachineBootstrapNetworkConfig) ([]*net.IPNet, error) {
	...
	// Parse configured addresses as subnets (supports CIDR, hyphenated ranges, and single IPs)
	subnets, rangeConstraints, err := subnetallocator.ParseIPRangesWithConstraints(config.Addresses)
	if err != nil {
		return nil, fmt.Errorf("failed to parse configured addresses: %w", err)
	}
	...
	// Parse gateway IPs to exclude them from allocation
	var excludeSubnets []*net.IPNet
	for _, gatewayStr := range config.Gateway {
		gatewayIP := net.ParseIP(gatewayStr)
		if gatewayIP == nil {
			return nil, fmt.Errorf("invalid gateway IP: %s", gatewayStr)
		}

		// Find the subnet that contains this gateway IP and create a /32 or /128 exclusion
		for _, subnet := range subnets {
			if subnet.Contains(gatewayIP) {
				...
				excludeSubnets = append(excludeSubnets, excludeSubnet)
				break
			}
		}
	}
