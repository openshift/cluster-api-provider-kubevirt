...
func buildVirtualMachineInstanceTemplateWithIPs(ctx *context.MachineContext, allocatedIPs []*net.IPNet) *kubevirtv1.VirtualMachineInstanceTemplateSpec {
	...
	// Add network data if NetworkConfig is specified
	if ctx.KubevirtMachine.Spec.NetworkConfig != nil {
		// Convert allocatedIPs to map by network name (assuming single network for now)
		allocatedIPsByNetwork := make(map[string][]*net.IPNet)
		if len(allocatedIPs) > 0 {
			// For backward compatibility, use the first network in IPPool
			for networkName := range ctx.KubevirtMachine.Spec.NetworkConfig.IPPool {
				allocatedIPsByNetwork[networkName] = allocatedIPs
				break
			}
		}

		networkDataJSON, err := EnsureCloudInitNetworkDataV2WithAllocatedIPs(ctx.KubevirtMachine.Spec.NetworkConfig, allocatedIPsByNetwork)
		if err != nil {
			ctx.Logger.Error(err, "Failed to process cloud-init network data, networkData will not be set in VirtualMachine")
		} else {
			// Set network data inline
			cloudInitVolume.VolumeSource.CloudInitConfigDrive.NetworkData = networkDataJSON
		}
	}
